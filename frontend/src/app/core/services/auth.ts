...
export class AuthService {
  private readonly api = 'http://localhost:8083';
  ...
  requestPasswordReset(email: string) {
    return this.http.post<{ message: string }>(`${this.api}/auth/password-reset/request`, { email });
  }

  resetPassword(token: string, password: string) {
    return this.http.post<{ message: string }>(`${this.api}/auth/password-reset/confirm`, { token, password });
  }
